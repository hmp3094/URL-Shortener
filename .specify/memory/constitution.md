...
- Version change: 1.2.0 → 2.0.0 (MAJOR: redefines/loosens previously-asserted MUST guarantees in
  Principles I, II, and IV — backward-incompatible relative to what those principles previously
  guaranteed)
- Reason for this amendment: this constitution was written broad and aspirational at project
  ...
  stats). The click-analytics feature actually built (no accounts, synchronous exact click
  counting, a much narrower analytics data set) diverged from it without ever being reconciled,
...
- Modified principles:
  - I. Short Link Integrity → the immutability guarantee no longer presumes an account/ownership
    ...
    last-accessed bookkeeping updates in place are named as an explicit, system-driven exception
    (not user-authenticated, since there's no ownership model). If an authenticated ownership
  ...
  - IV. Analytics Without Compromise → narrowed the mandated data set from
    ...
    never built.
- Modified sections:
  - Reliability & Data Standards → added a note that synchronous click tracking makes analytics-
    ingestion lag trivially ~0, satisfying the existing measure-and-document requirement rather
    than conflicting with it. The "expired/deleted/never-existed" distinguishability clause is
    untouched — this project has neither feature yet, so it's not in conflict here.
- Added sections: none
...
- NOT amended (explicitly, by project-owner instruction):
  - III. Test-First Delivery — left completely unchanged, still NON-NEGOTIABLE. Logged deviation:
    click-analytics was implemented with tests written after the code, not before, violating this
    principle as written. This is **not** brought into retroactive compliance by this amendment —
    it's logged here as an explicit, acknowledged violation per the Governance clause's
    "explicitly justified... plan to come into compliance." The plan: Test-First applies without
    exception to all work on this project from this point forward; no further exceptions will be
    logged silently. (The stats rate-limiting fix below was built this way: test written and
    confirmed failing — via manual curl verification, since Testcontainers doesn't run on this
...
- Resolved since this amendment was first drafted (fixed in code, not just documented):
  ...
  - The Reliability & Data Standards' load/latency-measurement requirement has been satisfied for
    this change — see `docs/performance.md`'s baseline-vs-click-tracking comparison.
- Still found during this audit, deliberately left open rather than silently resolved (raised to
  ...
  - Principle V's "contract before implementation" requirement — `docs/api.yaml` was updated
    alongside/after the click-analytics code, not before it, the same pattern as the Test-First
    deviation above.
- Follow-up TODOs:
